 ...
 * 双方篮板总和
 */
public class CustomGuessLB implements ICustomGuessBattle{
	
	@Override
	public CustomGuessResult getCustomGuessResult(BattleSource bs) {
		int home = bs.getHome().getPlayers().stream().mapToInt(player->(int)player.getRealTimeActionStats().getValue(EActionType.reb)).sum();
		int away = bs.getAway().getPlayers().stream().mapToInt(player->(int)player.getRealTimeActionStats().getValue(EActionType.reb)).sum();
		int total = home + away;
