...
// */
//@EventRegister({EEventType.装备升级})
//@ActiveAnno(redType=ERedType.活动, atv = EAtv.巨星之路_装备升级, clazz=AtvPlayerStarRoad.class)
//public class AtvStarLoadEquiUpLvManager extends AtvStarLoadBaseManager {
//	
//	@IOC
...
//				continue;
//			}
//			Integer lv = Integer.valueOf(cfgs.get(id).getConditionMap().get("lv"));
...
//		return equiManager.getTeamEqui(teamId).getTeamAllEqui().stream().filter(e-> e.getLv() >= needLv).mapToInt(e-> e.getLv()).sum();
//	}
//	
//	private int getTeamEquiCount(int needLv, long teamId) {
//		return (int) equiManager.getTeamEqui(teamId).getTeamAllEqui().stream().filter(e-> e.getLv() >= needLv).mapToInt(e-> e.getLv()).count();
//	}
//	
//}
